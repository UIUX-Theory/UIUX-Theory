#!/usr/bin/env python3
"""UI/UX 적용 예시 목업 생성 (SVG + HTML). 톤: grayscale + amber, gen_diagrams.py와 동일 팔레트."""
import os, textwrap

OUT = os.path.join(os.path.dirname(__file__), "assets", "examples")
os.makedirs(OUT, exist_ok=True)

BG, INK, MID, DOM, HI, FLOOR, AMBER, WHITE = (
    "#EBEBEB", "#5A5A5A", "#8C8C8C", "#B0B0B0",
    "#D2D2D2", "#707070", "#FFB919", "#FFFFFF",
)
W, H = 880, 460
...
def html_doc(title, body, extra_css=""):
    ...
    <style>
      :root {{
        ...
        --hi:{HI}; --floor:{FLOOR}; --amber:{AMBER}; --white:{WHITE};
      }}
...
for gtitle, fields in groups:
    s += f'<text x="500" y="{y}" font-size="11" fill="{AMBER}" font-weight="700" letter-spacing="1">{gtitle.upper()}</text>'
    y += 14
    for lab in fields:
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:48px">
      ...
      <div>
        ...
        <section style="margin-bottom:32px">
          <div style="color:var(--amber);font-weight:700;letter-spacing:1px;font-size:11px;margin-bottom:8px">계정</div>
        ...
        <section>
          <div style="color:var(--amber);font-weight:700;letter-spacing:1px;font-size:11px;margin-bottom:8px">프로필</div>
...
for i, lab in enumerate(["취소", "임시저장", "발행하기"]):
    x = 60 + i*150
    s += f'<rect x="{x}" y="80" width="130" height="40" rx="10" fill="{AMBER}"/>'
...
# secondary: text-like, primary: amber filled
...
s += f'<rect x="330" y="220" width="160" height="40" rx="10" fill="{AMBER}"/>'
...
for i, (lab, fill, fg) in enumerate([("진행중", AMBER, WHITE), ("진행중", AMBER, WHITE),
...
save("02-2-similarity-buttons", svg_frame(s), html_doc(
    ...
    <div style="display:flex;gap:12px;margin-bottom:32px">
      <button style="padding:10px 20px;border:0;border-radius:10px;background:var(--amber);color:#fff;font-weight:600">취소</button>
      <button style="padding:10px 20px;border:0;border-radius:10px;background:var(--amber);color:#fff;font-weight:600">임시저장</button>
      <button style="padding:10px 20px;border:0;border-radius:10px;background:var(--amber);color:#fff;font-weight:600">발행하기</button>
    </div>
    ...
    <div style="display:flex;gap:12px;align-items:center;margin-bottom:32px">
      ...
      <button style="padding:10px 24px;border:0;border-radius:10px;background:var(--amber);color:#fff;font-weight:700">발행하기</button>
    </div>
    <p style="color:var(--floor);font-weight:600">상태 태그는 종류별로 같은 색</p>
    <div style="display:flex;gap:8px">
      <span style="padding:4px 12px;border-radius:13px;background:var(--amber);color:#fff;font-size:11px;font-weight:600">진행중</span>
      <span style="padding:4px 12px;border-radius:13px;background:var(--amber);color:#fff;font-size:11px;font-weight:600">진행중</span>
...
save("02-3-common-region-cards", svg_frame(s), html_doc(
    ...
    "".join(f"""
      <section style="background:#fff;border:1px solid var(--hi);border-radius:14px;padding:20px">
        ...
        {"".join(f'<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0"><span>{lab}</span><span style="display:inline-block;width:42px;height:22px;border-radius:11px;background:{(AMBER if on else HI)};position:relative"><span style="position:absolute;top:3px;{("right:3px" if on else "left:3px")};width:16px;height:16px;border-radius:50%;background:#fff"></span></span></div>' for lab, on in rows)}
...
s += f'<line x1="{xs[0]}" y1="{y}" x2="{xs[1]}" y2="{y}" stroke="{AMBER}" stroke-width="4"/>'
for i, ((label, state), x) in enumerate(zip(steps, xs)):
    if state == "완료":
        s += f'<circle cx="{x}" cy="{y}" r="22" fill="{AMBER}"/>'
        ...
        s += f'<circle cx="{x}" cy="{y}" r="22" fill="{WHITE}" stroke="{AMBER}" stroke-width="3"/>'
        s += f'<text x="{x}" y="{y+5}" text-anchor="middle" font-size="13" font-weight="700" fill="{AMBER}">{i+1}</text>'
    else:
...
save("02-4-connectedness-stepper", svg_frame(s), html_doc(
    ...
    <div style="position:relative;display:flex;justify-content:space-between;padding:0 40px">
      ...
      <div style="position:absolute;left:60px;width:200px;top:21px;height:4px;background:var(--amber);z-index:0"></div>""" +
    "".join(f"""
      <div style="position:relative;z-index:1;text-align:center;width:80px">
        <div style="width:46px;height:46px;border-radius:50%;margin:0 auto;display:flex;align-items:center;justify-content:center;font-weight:700;{
            'background:var(--amber);color:#fff' if state=='완료'
            else 'background:#fff;border:3px solid var(--amber);color:var(--amber)' if state=='진행중'
...
s += f'<circle cx="0" cy="0" r="4" fill="{AMBER}"/><circle cx="14" cy="0" r="4" fill="{HI}"/><circle cx="28" cy="0" r="4" fill="{HI}"/><circle cx="42" cy="0" r="4" fill="{HI}"/></g>'
...
# CTA "더 읽기 ↓"
s += f'<rect x="170" y="350" width="170" height="44" rx="22" fill="{AMBER}"/>'
...
save("02-6-closure-readmore", svg_frame(s), html_doc(
    ...
    <div style="text-align:center;margin-top:16px">
      <button style="padding:12px 32px;border:0;border-radius:22px;background:var(--amber);color:#fff;font-weight:700;cursor:pointer">더 읽기 ↓</button>
    </div>
    """))
...
s += f'<rect x="{mx+mw-120}" y="{my+mh-58}" width="100" height="38" rx="10" fill="{AMBER}"/>'
      ...
      <div style="position:absolute;inset:0;background:rgba(112,112,112,.55);display:flex;align-items:center;justify-content:center">
        <div style="background:#fff;border-radius:16px;padding:24px;width:380px;box-shadow:0 12px 32px rgba(0,0,0,.25)">
          ...
          <div style="display:flex;justify-content:flex-end;gap:8px">
            ...
            <button style="padding:10px 24px;border:0;border-radius:10px;background:var(--amber);color:#fff;font-weight:700">삭제</button>
          </div>
        </div>
      </div>
    </div>
...
s += f'<rect x="500" y="110" width="280" height="34" rx="8" fill="{WHITE}" stroke="{AMBER}" stroke-width="2"/>'
s += f'<text x="520" y="132" font-size="13" font-weight="700" fill="{INK}">배송 정보</text>'
s += f'<text x="760" y="132" font-size="14" fill="{AMBER}">－</text>'
...
for i, lab in enumerate(["결제 수단", "주문 요약"]):
    ...
    s += f'<path d="M785 {yy-6} l0 18 m-5 -6 l5 6 5 -6" stroke="{AMBER}" stroke-width="2" fill="none" stroke-linecap="round"/>'
...
for i, (name, price, recommended) in enumerate(plans):
    ...
    for j in range(3):
        fy = y0 + 150 + j*32
        s += f'<circle cx="{x+30}" cy="{fy}" r="6" fill="{AMBER}"/>'
    ...
    # CTA
    bg = AMBER if recommended else WHITE
...
save("02-9-symmetry-pricing", svg_frame(s), html_doc(
    ...
    "".join(f"""
      <article style="position:relative;background:#fff;border:{('3px solid var(--amber)' if rec else '1px solid var(--hi)')};border-radius:16px;padding:24px;text-align:center">
        {('<span style="position:absolute;top:-14px;left:50%;transform:translateX(-50%);background:var(--amber);color:#fff;padding:4px 14px;border-radius:13px;font-size:11px;font-weight:700">추천</span>' if rec else '')}
        ...
        <button style="width:100%;padding:12px;border:{('0' if rec else '1.5px solid var(--dom)')};border-radius:10px;background:{('var(--amber)' if rec else '#fff')};color:{('#fff' if rec else 'var(--ink)')};font-weight:700">선택</button>
