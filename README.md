...
- 원리 설명 다이어그램은 모두 SVG로 제작되어 있어 Figma·일러스트레이터로 색/형태를 바로 수정할 수 있고, GitHub README에서도 인라인 렌더됩니다.
- **3장(위계 분석)·4.5절(이벤트 페이지)** 두 곳은 본인 작업 화면이 들어갈 자리라 임시 placeholder 이미지가 들어가 있습니다. 같은 파일명(`assets/03-hierarchy-annotated.svg`, `assets/04-5-event-page.svg`)으로 캡처를 덮어쓰면 교체됩니다.
